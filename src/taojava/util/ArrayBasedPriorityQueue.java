...
public class ArrayBasedPriorityQueue<T> implements PriorityQueue<T> {
    ...
    public ArrayBasedPriorityQueue(int capacity, Comparator<T> order) 
            throws Exception {
        if (capacity <= 0) {
    ...
    @Override
    /**
     * Build an iterator that returns the values of the priority
     * queue, but not necessarily in priority order.
     */
